 ...
 * è stato modificato il metodo searchForSingleEntry per adattarlo alle specifiche dell'applicazione
 * 
...
public class OppabLdapTemplate extends org.springframework.ldap.core.LdapTemplate {
    ...
    public OppabLdapTemplate(ContextSource contextSource) {
     ...
     * Il metodo è stato modificato per consentire la connessione a ldap 
    ...
    public DirContextOperations searchForSingleEntry(final String base, final String filter, final Object[] params) {
        ...
        return (DirContextOperations) executeReadOnly(new ContextExecutor() {
                public Object executeWithContext(DirContext ctx)
                	...
                	String username = context.getRequestParameterMap().get("j_username").trim();
                	String password = context.getRequestParameterMap().get("j_password");
                    ...
                    //System.out.println("OppabLdapTemplate: got some results");
                    if (!results.hasMore()) {
	 ...
	 */
	private static String readDomainParameter(){
		try
		{
			...
			return settingsBean.get_ldapDomain();
		}
		catch(Exception e){
			return "";
		}
